...
class PagesContent
{
    ...
    public function getMarkers($pageId, $templateId)
    {
        $sqlRes = $this->db->query('
        ...
        foreach ($sqlRes as $row) {
            ...
            if (empty($sqlRes2)) {
                $moduleTitle = str_replace('###1###', $row['page_content_type_id'], $this->translator->translate('Undefined method "###1###"'));
            } else {
                $moduleTitle = $this->translator->translateI18n($sqlRes2[0]['title']);
            }
        ...
        if (!empty($contentModules)) {
            ...
            $markers[] = array(
                'title' => $this->translator->translate('Markers from another templates'),
                'name' => '',
                'modules' => $tmp, 
            );
        }
