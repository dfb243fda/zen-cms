...
class Catalog
{
    ...
    protected $rubricGuid = 'catalog';
    protected $itemGuid   = 'product';
    ...
    {        
        ...
        if (null === $this->objectId) {      
            ...
            if (self::ITEM == $this->catalogType && $this->moduleManager->isModuleActive('Comments')) {
                $commentsService = $this->serviceManager->get('Comments\Service\Comments');
                $data['allow_comments'] = $commentsService->isAllowedComments($this->objectId);
            } 
    ...
    {
        if (self::RUBRIC == $this->catalogType) {
            $typeIds = $this->getRubricTypeIds();
        } else {
            $typeIds = $this->getItemTypeIds();
        }
    ...
    {        
        ...
        if (self::ITEM == $this->catalogType && $this->moduleManager->isModuleActive('Comments')) {
            $baseFormConfig['fieldsets']['common']['spec']['elements']['allow_comments'] = array(
                'spec' => array(
                    'type' => 'checkbox',
                    'name' => 'allow_comments',
                    'options' => array(
                        'label' => $this->translator->translate('Comments:Allow comments'),
                    ),
                ),
            );
        }               
    ...
    public function edit($data)
    {
        ...
        foreach ($data as $k=>$v) {
            if (isset($v['name']) && '' == $v['name']) {
                $data[$k]['name'] = $this->translator->translate('Catalog:(Without name)');
            }
        }
