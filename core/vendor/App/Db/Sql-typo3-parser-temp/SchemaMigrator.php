...
class SchemaMigrator {
	...
	public function __construct($options) {
        ...
        if (isset($options['defaultCharset'])) {
				...
				if (is_array($cfg['fields'])) {
					foreach ($cfg['fields'] as $fN => $fType) {
						...
						if ($sqlParser->parse_error) {
							throw new \RuntimeException('Zen-CMS Fatal Error: ' . $sqlParser->parse_error, 1270853961);
						}
					}
				}
			}
				...
				if (!strlen($onlyTableList) || GeneralUtility::inList($onlyTableList, $table)) {
					if (!isset($FDcomp[$table])) {
						...
						foreach ($keyTypes as $theKey) {
							if (isset($info[$theKey]) && is_array($info[$theKey])) {
				...
				foreach ($diffArr[$theKey] as $table => $info) {
					...
					if (is_array($info['fields'])) {
						foreach ($info['fields'] as $fN => $fV) {
							if (isset($info['whole_table']) && $info['whole_table']) {
								$whole_table[] = $fN . ' ' . $fV;
							} else {
							...
							if (!$remove) {
								if (!isset($info['whole_table']) || !$info['whole_table']) {
									...
									if ($fN == 'CLEAR') {
										...
										if ((isset($info['keys']) && count($info['keys'])) || $fV == 2) {
											$clear_table = TRUE;
										}
										continue;
									} else {
		...
		if (is_array($arr)) {
			foreach ($arr as $key => $string) {
				if (isset($keyArr[$key]) && $keyArr[$key]) {
					$res = $this->db->query($string, array());
...
//					}
	...
	public function getListOfTables() {
        $whichTables = $this->admin_get_tables();
    ...
    protected function admin_get_tables()
    {
        $whichTables = array();
        
        $dbName = $this->db->getCurrentSchema();
        
		$tables_result = $this->db->query('SHOW TABLE STATUS FROM `' . $dbName . '`', array())->toArray();
        foreach ($tables_result as $theTable) {
    ...
    protected function admin_get_fields($tableName) {
		$output = array();
		$columns_res = $this->db->query('SHOW COLUMNS FROM `' . $tableName . '`', array())->toArray();
        foreach ($columns_res as $fieldRow) {
            $output[$fieldRow['Field']] = $fieldRow;
        }
		return $output;
	}
    
    protected function admin_get_keys($tableName) {
        $output = array();
        $keyRes = $this->db->query('SHOW KEYS FROM `' . $tableName . '`', array())->toArray();
        foreach ($keyRes as $keyRow) {
            $output[] = $keyRow;
        }
        return $output;
    }
    
    protected function exec_SELECTcountRows($field, $table, $where = '') {        
        if ($where) {
            $where = ' where ' . $where;
        }
        
        $sqlRes = $this->db->query('select count(' . $field . ') as cnt from ' . $table . $where, array())->toArray();
        
        return (int)$sqlRes[0]['cnt'];
	}
}
